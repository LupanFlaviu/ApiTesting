...
describe("epoch-to-unix-tests", () => {
  it("Conversion from Epoch to Unix AM is done successfully", function (done) {
    var unixDate = Math.floor(new Date(amEpochValue) / 1000)
      .toString()
      .replace(' "undefined"', "");
    var diferenceLocalTimeAndUtc =
      new Date(amEpochValue).getTimezoneOffset() / -1;
    var finalUnixDateAm = Number(unixDate) + diferenceLocalTimeAndUtc * 60;

    request.get(amEpochValue).expect("" + finalUnixDateAm + "");
    done();
  });
});
...
describe("epoch-to-unix-tests", () => {
  it("Conversion from Epoch to Unix PM is done successfully", function (done) {
    var unixDate = Math.floor(new Date(pmEpochValue) / 1000)
      .toString()
      .replace(' "undefined"', "");
    var diferenceLocalTimeAndUtc =
      new Date(amEpochValue).getTimezoneOffset() / -1;
    var finalUnixDate =
      Number(unixDate) + Number(diferenceLocalTimeAndUtc * 60);
    request.get(pmEpochValue).expect("" + finalUnixDate + "");
    done();
  });
});
